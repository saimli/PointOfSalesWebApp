            ...
            if(rows1.length == 0) {
                ...
                if(!req.session.userID) {
                    ...
                    request.query(query, function(err, rows2) {
                        ...
                        res.render('supsoldReport', {data: rows1.recordsets[0], userID: userID, isAdmin: null});
                    })
                }  
                else{
                    res.render('supsoldReport', {data: rows1.recordsets[0], userID: req.session.userID, isAdmin: req.session.isAdmin})
                } 
            }
        })
    })
